...
package com.ioteye.gta.flink.mcep.pattern;
...
import com.ioteye.gta.flink.mcep.pattern.conditions.BooleanConditions;
import com.ioteye.gta.flink.mcep.pattern.conditions.IterativeCondition;
import com.ioteye.gta.flink.mcep.pattern.conditions.RichAndCondition;
import com.ioteye.gta.flink.mcep.pattern.conditions.RichOrCondition;
import com.ioteye.gta.flink.mcep.pattern.conditions.SubtypeCondition;
 ...
 *
 * <p>A pattern definition is used by {@link NFACompiler} to create a {@link NFA}.
 *
 * <p><pre>{@code
...
public class Pattern<T, F extends T> {
	 ...
	 *
	 * @return The same pattern with a {@link Quantifier#looping(Quantifier.ConsumingStrategy)} quantifier applied.
	 * @throws MalformedPatternException if the quantifier is not applicable to this pattern.
	 */
	...
	/**
	 * Applicable only to {@link Quantifier#looping(Quantifier.ConsumingStrategy)} and
	 * {@link Quantifier#times(Quantifier.ConsumingStrategy)} patterns, this option allows more flexibility to the matching events.
	 *
	 ...
	 * <pre>{@code
	 * Pattern.<InjectEvent>begin("start").where(new SimpleCondition<InjectEvent>() {
	 *      @Override
	 *      public boolean filter(InjectEvent value) throws Exception {
	 *          return value.getName().equals("c");
	 *      }
	 * })
	 * .followedBy("middle").where(new SimpleCondition<InjectEvent>() {
	 *      @Override
	 *      public boolean filter(InjectEvent value) throws Exception {
	 *          return value.getName().equals("a");
	 *      }
	 * }).oneOrMore().consecutive()
	 * .followedBy("end1").where(new SimpleCondition<InjectEvent>() {
	 *      @Override
	 *      public boolean filter(InjectEvent value) throws Exception {
	...
	public static <T, F extends T> GroupPattern<T, F> begin(Pattern<T, F> group) {
		return new GroupPattern<>(null, group, Quantifier.ConsumingStrategy.STRICT, AfterMatchSkipStrategy.noSkip());
	}
	...
	public GroupPattern<T, F> followedBy(Pattern<T, F> group) {
		return new GroupPattern<>(this, group, Quantifier.ConsumingStrategy.SKIP_TILL_NEXT, afterMatchSkipStrategy);
	}
	...
	public GroupPattern<T, F> followedByAny(Pattern<T, F> group) {
		return new GroupPattern<>(this, group, Quantifier.ConsumingStrategy.SKIP_TILL_ANY, afterMatchSkipStrategy);
	}
