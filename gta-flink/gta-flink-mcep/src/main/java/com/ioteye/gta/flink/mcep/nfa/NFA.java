...
package com.ioteye.gta.flink.mcep.nfa;

import com.ioteye.gta.flink.mcep.nfa.aftermatch.AfterMatchSkipStrategy;
import com.ioteye.gta.flink.mcep.nfa.sharedbuffer.EventId;
import com.ioteye.gta.flink.mcep.nfa.sharedbuffer.NodeId;
...
import com.ioteye.gta.flink.mcep.operator.CepOperator;
 ...
 *
 * <p>The {@link CepOperator GtaCEP operator}
 ...
 * {@link com.ioteye.gta.flink.mcep.nfa.sharedbuffer.SharedBuffer buffer}, which is a memory-optimized data-structure exactly for
 ...
 * </ol>
 *
 * <p>The implementation is strongly based on the paper "Efficient Pattern Matching over InjectEvent Streams".
 *
...
public class NFA<T> {
	 ...
	 * {@link Pattern#within(Time)}  Pattern.within(Time)}
	 * method.
	 */
	 ...
	 * 	   	<ol>
	 *          <li>IGNORE (links in {@link com.ioteye.gta.flink.mcep.nfa.sharedbuffer.SharedBuffer} will still point to the previous event)</li>
	 *          <ul>
	 ...
	 *          	<li>lock the entry in {@link com.ioteye.gta.flink.mcep.nfa.sharedbuffer.SharedBuffer} as it is needed in the created branch</li>
	 *      	</ul>
	 *      	<li>TAKE (links in {@link com.ioteye.gta.flink.mcep.nfa.sharedbuffer.SharedBuffer} will point to the current event)</li>
	 *          <ul>
	 ...
	 *     <li>Release the corresponding entries in {@link com.ioteye.gta.flink.mcep.nfa.sharedbuffer.SharedBuffer}.</li>
	 *</ol>
	 *
	 ...
	 *
	 * @param sharedBufferAccessor The accessor to {@link com.ioteye.gta.flink.mcep.nfa.sharedbuffer.SharedBuffer} from which to extract the matches
	...
	public static class MigratedNFA<T> {
		...
		private final com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T> sharedBuffer;

		public com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T> getSharedBuffer() {
			return sharedBuffer;
		}
		...
		MigratedNFA(
				...
				final com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T> sharedBuffer) {
	...
	public static final class NFASerializerConfigSnapshot<T> extends CompositeTypeSerializerConfigSnapshot<MigratedNFA<T>> {
		...
		public NFASerializerConfigSnapshot(
				...
				TypeSerializer<com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T>> sharedBufferSerializer) {
	...
	public static final class MigratedNFASerializerSnapshot<T> extends CompositeTypeSerializerSnapshot<MigratedNFA<T>, NFASerializer<T>> {
		...
		protected NFASerializer<T> createOuterSerializerWithNestedSerializers(TypeSerializer<?>[] nestedSerializers) {
			...
			@SuppressWarnings("unchecked")
			TypeSerializer<com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T>> sharedBufferSerializer =
				(TypeSerializer<com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T>>) nestedSerializers[1];
	...
	public static class NFASerializer<T> extends TypeSerializer<MigratedNFA<T>> {
		...
		private final TypeSerializer<com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T>> sharedBufferSerializer;

		private final TypeSerializer<T> eventSerializer;

		public NFASerializer(TypeSerializer<T> typeSerializer) {
			this(typeSerializer,
				new com.ioteye.gta.flink.mcep.nfa.SharedBuffer.SharedBufferSerializer<>(
		...
		public MigratedNFA<T> deserialize(DataInputView source) throws IOException {
			...
			com.ioteye.gta.flink.mcep.nfa.SharedBuffer<T> sharedBuffer = sharedBufferSerializer.deserialize(source);
			Queue<ComputationState> computationStates = MigrationUtils.deserializeComputationStates(sharedBuffer, eventSerializer, source);
