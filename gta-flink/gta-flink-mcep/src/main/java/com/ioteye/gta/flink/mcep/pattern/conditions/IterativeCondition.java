...
import com.ioteye.gta.flink.mcep.time.TimeContext;
 ...
 * <pre>
 * {@code
 * private class MyCondition extends IterativeCondition<InjectEvent> {
 *
 * 		@Override
 *     	public boolean filter(InjectEvent value, Context<InjectEvent> ctx) throws Exception {
 *     		if (!value.getName().equals("middle")) {
 *     			return false;
 *     		}
 *
 *     		double sum = 0.0;
 *     		for (InjectEvent e: ctx.getEventsForPattern("middle")) {
 *     			sum += e.getPrice();
 *     		}
 *     		sum += value.getPrice();
