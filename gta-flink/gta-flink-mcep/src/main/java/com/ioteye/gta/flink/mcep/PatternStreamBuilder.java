 ...
 */
@Internal
final class PatternStreamBuilder<IN extends InjectEvent> {

	private final DataStream<IN> inputStream;

	private final InjectionPatternFunction<IN> function;
	...
	private PatternStreamBuilder(
			...
			final InjectionPatternFunction<IN> function,
	...
	PatternStreamBuilder<IN> withComparator(final EventComparator<IN> comparator) {
		return new PatternStreamBuilder<>(inputStream, function, checkNotNull(comparator), lateDataOutputTag);
	}

	PatternStreamBuilder<IN> withLateDataOutputTag(final OutputTag<IN> lateDataOutputTag) {
		return new PatternStreamBuilder<>(inputStream, function, comparator, checkNotNull(lateDataOutputTag));
	}
	...
	static <IN extends InjectEvent> PatternStreamBuilder<IN> forStreamAndPattern(final DataStream<IN> inputStream, final InjectionPatternFunction<IN> function) {
		return new PatternStreamBuilder<>(inputStream, function, null, null);
	}
}
